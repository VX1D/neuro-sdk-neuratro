...
local check, done = require("tests.helpers").harness("consumables")
...
do
  local c = mk("c_test", "Tarot", 3)
  c.ability.consumeable.mod_num = 2
  local mn, mx = CardUtil.consumable_target_range(c)
  check("target range follows the game's mod_num when present", mx == 2 and mn == 1,
    tostring(mn) .. "," .. tostring(mx))
  local c2 = mk("c_test2", "Tarot", 9)
  local _, mx2 = CardUtil.consumable_target_range(c2)
  check("without mod_num the range is capped at 5 like set_ability", mx2 == 5, tostring(mx2))
  local c3 = mk("c_test3", "Tarot", 2)
  c3.ability.consumeable.min_highlighted = 2
  local mn3 = CardUtil.consumable_target_range(c3)
  check("min_highlighted is honoured", mn3 == 2, tostring(mn3))
end
