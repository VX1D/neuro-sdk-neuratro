...
check("immediate resend commits (never a trap)", DW.evaluate("leave_shop") == false)

shop(12, 4)
DW.evaluate("leave_shop")
DW.acknowledge("record_plan")
check("rewriting the money plan also clears it", DW.would_reject("leave_shop") == false)

shop(12, 6)
check("leaving with a full step -> silent", DW.evaluate("leave_shop") == false)
shop(12, 9)
check("remainder above a step ($9) -> silent, not a remainder nag",
  DW.evaluate("leave_shop") == false)
shop(4, 4)
check("arrived poor and stayed poor -> silent (nothing was forfeited)",
  DW.evaluate("leave_shop") == false)
shop(12, 4, { modifiers = { no_interest = true } })
check("no_interest run -> silent", DW.evaluate("leave_shop") == false)
shop(12, -3, { bankrupt_at = -20 })
check("Credit Card overdraft -> silent (never pressure a buy under water)",
  DW.evaluate("leave_shop") == false)
shop(12, 4, { interest_amount = 0 })
check("B6a interest_amount 0 -> silent (claim would be untrue)", DW.evaluate("leave_shop") == false)
shop(12, 4, { interest_cap = 0 })
check("B6b interest_cap below one step -> silent", DW.evaluate("leave_shop") == false)
shop(12, 0)
check("leaving broke still fires (0 is below the step, and this shop caused it)",
  DW.evaluate("leave_shop") == "soft_reject")

shop(12, 4)
DW.evaluate("leave_shop")
DW.evaluate("leave_shop")
G.NEURO.shop_visit_epoch = 2
check("next shop visit re-arms", DW.evaluate("leave_shop") == "soft_reject")

shop(12, 4, { interest_amount = 2, interest_cap = 50 })
local _, m2 = DW.evaluate("leave_shop")
...
shop(11, 4)
check("entered $11 -> below the entry threshold, no gate", DW.evaluate("leave_shop") == false)
shop(12, 4)
check("entered $12 -> at the entry threshold, gate arms",
  DW.evaluate("leave_shop") == "soft_reject")
shop(30, 4)
check("entered $30 -> well above, gate arms", DW.evaluate("leave_shop") == "soft_reject")
