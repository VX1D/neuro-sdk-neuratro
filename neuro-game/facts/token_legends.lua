...
local READABLE_SELECTING_HAND =
  ...
  .. "Playing a hand spends nothing until the commit goes through: an unguarded play_hand commits immediately, while a guarded play_hand opens one transaction. Once that transaction is delivered, only resolve_play is valid: echo its integer transaction_id with answer:\"yes\" to commit the exact named cards or answer:\"no\" to cancel it. Repeating play_hand, discard_hand, or another hand mutator cannot replace or confirm the open transaction. No itself never plays, discards, or draws. Answering no spends the single review for this unchanged hand: use discard_hand next for a redraw, or play_hand for a specific alternative; that next play is final and commits immediately. An optional reason is repeated in that final-choice context. "
...
local READABLE_COMMON =
  "How to decide: read the situation and your available actions, drop any blocked by money, slots, hands, discards, or targets, then call exactly ONE action type. Confirming a play means calling resolve_play with the exact transaction_id shown in the prompt and yes or no; answer yes commits the named cards, answer no cancels the proposal without discarding cards. Confirming a sell means sending that same action again with identical arguments -- that is the commit, not a second move. "
